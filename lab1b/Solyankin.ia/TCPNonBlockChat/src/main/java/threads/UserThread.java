...
//                int messageLength = Integer.parseInt(new String(buffer.array()).trim());
...
//                                key.interestOps(SelectionKey.OP_WRITE);
                                if (!message.equals("")) {
                                    if (message.split(" ", 2)[0].equals(Phrases.CLIENT_COMMAND_FILE.getPhrase())) {
                                        ...
                                        buffer.limit((user.getUserName() + " " + message).length());
                                        buffer.clear();
                                        buffer = ByteBuffer.wrap(((user.getUserName() + " " + message).trim()).getBytes());
                                        channel.write(buffer);
    ...
    private void sendFile(String filePath, SocketChannel channel) throws IOException {
        ...
        if (file.length() > 5 * 1024 * 1024) {
            ...
            if (file.exists() && !file.isDirectory()) {
                ...
                buffer = ByteBuffer.wrap((user.getUserName() + " " + Phrases.CLIENT_COMMAND_FILE.getPhrase() + " " + file.getName().length() + " " + file.getName() + " " + fileBytes.length + " ").getBytes());
                channel.write(buffer);
