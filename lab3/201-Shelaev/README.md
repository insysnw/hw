...
Команды **4** - **8** - только для авторизованных пользователей.  
Команда **4** выполняется автоматически с командой **1**, если до этого был выполнен вход пользователя в систему.

Стоит отметить, что одно из условий выполнения команды **8** является не превышение суммы перевода значения на электронном кошельке,   
с которого осуществляется этот перевод.

### Сообщения пользователю
Все команды выше сопровождаются различными сообщениями.  
При успешном запросе пользователю отображается полученная информация, например, состояние его счёта   
(сообщение: `In the wallet {wallet_num} You have {account}`).  
После корректного выполнения команд `login` или `logout` пользователь увидит сообщение следующего формата:   
`OK: You have successfully logged in (out)`.

Если неавторизованный пользователь захочет выполнить команды **4** - **8**, то он получит предупреждение:   
`Warning! You are not logged in and most of the commands are unavailable`.   
С другой стороны, если авторизованный пользователь попробует выполнить команду **2** или **3**, то появится сообщение:   
...
обрабатываются соответствующим образом в зависимости от введённой команды. Например, при ошибке **404** может появиться сообщение:   
